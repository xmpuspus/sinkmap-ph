...
- `web/` - single-file static map (index.html, MapLibre, no-key Carto basemap, PNG
  image-overlays). serve.py is Range-capable (port 8788). See "Web UI" below.
...
## Web UI (web/index.html, single file)

First-touch is a bottom STORY RAIL that leads with one finding and walks the rest
(prev/next; tap = fly + analysis layer + callouts; preview-until-engaged, then the
walk drives the map). The `#findings` left drawer is the "see all" list. Both reuse
`selectFinding()`. There is also find-your-city (`#findcity` select), a top-center
mode toggle (Rate / Watch it sink), and the velocity legend.

- Findings come from `web/data/findings.json`, generated by
  `scripts/make_finding_layers.py` (mintpy312 env, reads `tmp/analysis/*.json`). Each
  finding = a LOCATION DATELINE (the `tag`, e.g. "Metro Manila / Bulacan", "Nationwide"),
  a news-style HEADLINE (`title`), a `stat`, and a plain-English `blurb`. EN/TL throughout.
  NEVER hand-type findings.json: edit the templates in make_finding_layers.py and
  regenerate (numbers are interpolated from the gated analysis JSON). The blurb is shown
  by default in the drawer + rail (rail clamps to 3 lines on phones).
- Analysis PNG layers (accel, tilt) are colorized from `data/insar/<city>/*.tif` by the
  same script. Accel uses PuOr (orange=speeding up, purple=slowing): colorblind-safe, NOT
  red/green. Changing accel colors means updating the colormap AND the CSS `RAMP` gradient
  AND the blurb's color words together.
- RESILIENCE (do not regress): render the findings TEXT (`renderFindings`/`renderRail`)
  BEFORE adding map layers, guard each `addLayer` on its own try, retry the findings fetch,
  and never silent-blank (show `CN.load_fail`). A `map.addSource/addLayer` throw must not
  blank the panels (Brave hit this; Chromium did not). agent-browser/Chromium passing does
  NOT prove other engines won't throw on map ops.
- e2e: `tests/e2e.sh` (124 checks) drives the real map via agent-browser against
  `window.__diag` / `window.map` / DOM. Hooks: ready, cities, layers, findings,
  findingLayers, activeFinding, callouts, railOpen, railIndex, findingsError. Extend it for
  every new element; pin computed values; never loosen. agent-browser routing honors
  `--abort`, NOT `--body`. Mobile = the rail is a bottom-sheet; controls dock above it via
  `layoutMobile()` height math; place cards slide over the rail.

## Deploy

Push to main = Vercel prod deploy (auto; project "web" under xmpuspus-projects, PERSONAL
account). The clean alias must be re-pointed manually every deploy:
`cd web && vercel alias set <newest web-XXXX deploy> sinkmap-ph.vercel.app` (grab the
newest from `vercel ls`). Verify-live = screenshot-read-back (Read the PNG, not just 200s;
passing checks hid bugs 3x this project) + `make e2e BASE=https://sinkmap-ph.vercel.app`.
After deploy, fingerprint the live HTML/JSON for the new copy. `docs/verify/` is QA scratch
(gitignored).
